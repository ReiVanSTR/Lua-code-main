...
local mNameSuck = meSuck.getInterfaceConfiguration(1)
-- няняяя
 mNameSuck = {id=mNameSuck.name}
-- Tak samo dla wypłaty środków
local mNameDrop = meSuck.getInterfaceConfiguration(1)
...
-- Otrzymanie: name = imię eventu, nil - to, co nie trzeba, code = numer przeciska  klawiaturzy
    ...
    -- Wypłata środków
        if code==74 then
            gpu.set(1, 15, "Вы вывели "..money.." железа!         ")
                piece = money/64
                pieceI = money%64
...
function random()
 ...
 -- Sprawdzanie zwycięstwa
        ...
        if rNumber==1 then
            ...
            wBet=bet*30
            allBet=allBet + wBet
            chat.say("§4Игрок §3"..pzn.."§3 сорвал Jackpot и выиграл §2"..wBet.." §3железа(о)!")
            wBet=wBet-bet
            os.sleep(0.2)
            if wBet > 64 then
                piece = wBet/64
                ...
                piece = math.floor(piece)
                for i=1, piece do
                    meDrop.exportItem(mNameDrop,"EAST", 64)
                    os.sleep(0.15)
                end
                meDrop.exportItem(mNameDrop,"EAST", pieceI)
            else
                meDrop.exportItem(mNameDrop,"EAST", wBet)
            end
            os.sleep(0.5)  
        elseif rNumber==19 or rNumber==28 then
            ...
            allBet=allBet + wBet  
            if a==2 then
                chat.say("§3Братишка тут выиграл §2"..wBet.." §3железа(о)§3, попробуй и ты! ")
            elseif a==3 then
                chat.say("§4"..pzn.." §3стал богаче на §2"..wBet.." §3железа(о)§3, попробуй и ты! ")
            elseif a==4 then
                chat.say("§3Парниши, сегодня §4"..pzn.." §3нас угщает, у него + §2"..wBet.." §3железа(о)!")
            else
                chat.say("§4"..pzn.." §3последовал моей тактике и выиграл §2"..wBet.." §3железа(о)")
            end
            wBet=wBet-bet
            os.sleep(0.2)
            if wBet > 64 then
        ...
        elseif rNumber== 27 or rNumber==36 then
            ...
            if bet > 64 then
                p=bet%64
                meSuck.exportItem(mNameSuck, "DOWN", 64)
                os.sleep(0.15)
                meSuck.exportItem(mNameSuck, "DOWN", p)
            else
            meSuck.exportItem(mNameSuck, "DOWN", bet)
            end
